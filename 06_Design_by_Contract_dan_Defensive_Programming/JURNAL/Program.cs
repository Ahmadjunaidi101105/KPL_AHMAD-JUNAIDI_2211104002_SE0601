// Menambahkan kode awal SAYTUBEVIDEO
// using System;
...
class Program
{
    static void Main()
    {
        ...
        foreach (var film in filmList)
        {
            ...
            // Menambah play count dengan angka valid
            try
            {
                video.IncreasePlayCount(100000); // Menambahkan 100.000 views
            }
            catch (Exception ex)
            {
        ...
        // Uji coba exception untuk overflow dengan batas yang diperbolehkan
        try
        {
            SayaTubeVideo testVideo = new SayaTubeVideo("Video Uji Overflow");
            for (int i = 0; i < 10; i++)
            {
                testVideo.IncreasePlayCount(25000000); // Maksimal yang diperbolehkan
            }
        }
        catch (Exception ex)
        {
